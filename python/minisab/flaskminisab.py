...
def render_template_categorie(id_categorie):
    ...
    if cat is not None:
        items = newminisab.recuperer_tous_articles_pour_une_categorie(cat.nom)
        return (
            render_template("./categorie.html", categorie=cat, items=items),
            render_template("./categorie_end.html", categorie=cat, items=items),
        )
    else:
        return ""
...
def check_urls():
    ...
    for termine_id, termine_fichier in resultat["article_termine"]:
        ...
        url = current_app.config["MINISABINV_URL_EXTERNE"] + url_for(
            "minisab.get_nzb", id_article=termine_id
        )
        logger.debug("ajout recherche tous pour %s et %s", termine_fichier, url)
...
def marquer_article_favoris_categorie(id_article=None):
    ...
    try:
        ...
        ar.marquer_favoris()
        ar.lancer_recherche()
        logger.info("marquer favoris %d nb recherche %d", id_article, len(ar.recherche))
        ar.save()
...
def marquer_article_favoris(id_article=None):
    ...
    try:
        ar = newminisab.Article.get(newminisab.Article.id == id_article)
        ar.marquer_favoris()
        logger.info("marquer favoris %d", id_article)
        ar.save()
        return render_template(
            "./article.html",
            item=ar,
        ...
        if request.method == "POST":
            ...
            for art_id in data_json:
                ...
                for y in ar.recherche:
                    logger.info(
                        "article_lu, title %s, id sab %s", ar.title, y.id_sabnzbd
                    )
                    if y.id_sabnzbd != "":
...
@bp.route("/article/<id_article>/recherche", defaults={"stop_multi": 0})
...
def lancer_telecharger(id_recherche, categorie):
    ...
    try:
        ...
        res_tele = sabnzbd_util.telechargement_sabnzbd(
            rec.article.title, rec.url, categorie
        )
...
@bp.route("/article/<int:id_article>/tout_telecharger/", defaults={"categorie": "*"})
def lancer_tout_telecharger(id_article, categorie):
    logger.info("Requete %s", request.url)
    try:
        rec = newminisab.Recherche.select().where(
            newminisab.Recherche.article == id_article
        )
...
def categorie_historique(id_categorie=None, taille_bloc=100, num_bloc=0):
    ...
    articles, nb_bloc = newminisab.recuperer_tous_articles_pour_une_categorie_lu(
        obj_categorie.nom, num_bloc, taille_bloc
    )

    return render_template(
        "./minifluxlistcomplete.html",
...
def flaskconfig():
    ...
    if request.method == "POST":
        ...
        logger.debug(
            "change config : %s", str([data_json[x] for x in data_json.keys()])
        )
        ...
        logger.info(
            ...
            settings.ConfigBase.strConfig(current_app.config),
        )

    config_app = settings.ConfigBase.get_config(current_app)
    return render_template("./config.html", config=config_app)
